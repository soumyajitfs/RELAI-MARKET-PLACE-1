...
export const buildEwsShapData = (row) => {
  ...
  const features = row.shapValues.map((sv) => {
    ...
    /** EWS API now returns the feature value directly inside each shap entry. */
    const apiValue = sv?.value ?? sv?.Value;
    const cellValue = apiValue !== undefined ? apiValue : (rowKey != null ? row[rowKey] : null);
