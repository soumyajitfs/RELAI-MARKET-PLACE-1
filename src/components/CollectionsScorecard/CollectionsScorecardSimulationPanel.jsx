...
import {
  fetchCollectionsScorecardAccounts,
  predictCollectionsScorecardAccounts,
} from '../../utils/collectionsScorecardApi';
import { buildCollectionsScorecardShapData } from '../../utils/collectionsScorecardShapUtils';
import {
  resolveCollectionsScorecardFieldRules,
  validateCollectionsScorecardValue,
  validateCollectionsScorecardRowCrossFields,
...
import CollectionsScorecardDataTable from './CollectionsScorecardDataTable';
...
const CollectionsScorecardSimulationPanel = () => {
  ...
  const loadRows = useCallback(async () => {
    ...
    setApiError(null);
    try {
      const data = await fetchCollectionsScorecardAccounts();
      setRows(data);
    } catch (err) {
      console.error('Failed to fetch collections accounts:', err);
  ...
  const fieldRulesWithEdit = useMemo(() => {
    const base = resolveCollectionsScorecardFieldRules();
    const sample = rows[0];
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const results = await predictCollectionsScorecardAccounts(rowsToRun);
  ...
  const validateField = useCallback(
    (field, value) => validateCollectionsScorecardValue(field, value, fieldRulesWithEdit),
    [fieldRulesWithEdit],
  );
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFieldRules({
      ...
      validateValue: validateCollectionsScorecardValue,
      validateRowCross: validateCollectionsScorecardRowCrossFields,
      isValuePresent,
  ...
  const shapData = useMemo(() => {
    ...
    if (!row) return null;
    return buildCollectionsScorecardShapData(row);
    ...
    <div className="simulation-panel">
      ...
      {isInitialLoading ? (
        <div className="loading-spinner">
          ...
          <p className="loading-text">Loading collections accounts from backend...</p>
        </div>
      ) : apiError ? (
        ...
        <div className="loading-spinner">
          <div className="spinner"></div>
          <p className="loading-text">Running Collections Scorecard model...</p>
        </div>
      ) : (
        <>
          <CollectionsScorecardDataTable
            rows={rows}
            draftRows={draftRows}
...
export default CollectionsScorecardSimulationPanel;
