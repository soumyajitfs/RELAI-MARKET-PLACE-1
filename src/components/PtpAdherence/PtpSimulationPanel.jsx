    ...
    for (const row of draftRows) {
      for (const [field, rule] of Object.entries(fieldRulesWithEdit)) {
        ...
        if (!isValuePresent(row[field])) {
          actions.showToast({
            message: `"${rule.label}" cannot be empty. (PTP ${row.ptp_id})`,
            type: 'warning',
          });
          return;
        }
        const message = validatePtpValue(field, row[field], fieldRulesWithEdit);
        if (message) {
          actions.showToast({
            message: `${message} (PTP ${row.ptp_id})`,
            type: 'warning',
          });
          return;
        }
      }
      const cross = validatePtpRowCrossFields(row);
      if (cross) {
        actions.showToast({
          message: `${cross} (PTP ${row.ptp_id})`,
          type: 'warning',
        });
        return;
      }
