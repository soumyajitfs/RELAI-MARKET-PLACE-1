...
import { groupDirtyCellsByRow } from '../../utils/dirtyCellTracking';
...
const CustomerLifecycleSimulationPanel = ({ generateKey, modelTitle }) => {
  ...
  const validateDraft = useCallback(() => {
    if (!fieldList || dirtyCells.size === 0) return true;

    for (const [rowId, fields] of groupDirtyCellsByRow(dirtyCells)) {
      const ri = draftRows.findIndex((row, i) => getRowId(row, i) === rowId);
      if (ri < 0) continue;
      const row = draftRows[ri];
      const acct = getRowId(row, ri);

      for (const spec of fieldList.fields) {
        if (spec.readOnly || !fields.has(spec.key)) continue;
    ...
    return true;
  }, [fieldList, draftRows, dirtyCells, actions]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const updateDraftCell = (rowIndex, key, newVal) => {
    setDraftRows((prev) => {
      ...
      if (row && markCellDirty) markCellDirty(getRowId(row, rowIndex), key);
      return prev.map((r, i) => (i !== rowIndex ? r : { ...r, [key]: newVal }));
    });
  };
