...
import { validateDirtyFields } from '../../utils/dirtyCellTracking';
...
const CollectabilitySimulationPanel = () => {
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFields(
      dirtyCells,
      draftRows,
      (field, value) => validateCollectabilityValue(field, value),
      ({ row, message }) => `${message} (Account ${row['CLIENT REFERENCE NUMBER']})`
    );
    if (error) {
