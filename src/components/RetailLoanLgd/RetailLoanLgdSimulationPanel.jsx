...
import { fetchRetailLoanLgdAccounts, predictRetailLoanLgdAccounts } from '../../utils/retailLoanLgdApi';
import { buildRetailLoanLgdShapData } from '../../utils/retailLoanLgdShapUtils';
import {
  resolveRetailLoanLgdFieldRules,
  validateRetailLoanLgdValue,
  validateRetailLoanLgdRowCrossFields,
...
import RetailLoanLgdDataTable from './RetailLoanLgdDataTable';
...
const RetailLoanLgdSimulationPanel = () => {
  ...
  const loadRows = useCallback(async () => {
    ...
    setApiError(null);
    try {
      const data = await fetchRetailLoanLgdAccounts();
      setRows(data);
    } catch (err) {
  ...
  const fieldRulesWithEdit = useMemo(() => {
    const base = resolveRetailLoanLgdFieldRules();
    const sample = rows[0];
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const results = await predictRetailLoanLgdAccounts(rowsToRun);
      ...
      console.error('Retail Loan LGD prediction failed:', err);
  ...
  const validateField = useCallback(
    (field, value) => validateRetailLoanLgdValue(field, value, fieldRulesWithEdit),
    [fieldRulesWithEdit]
  );
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFieldRules({
      ...
      validateValue: validateRetailLoanLgdValue,
      validateRowCross: validateRetailLoanLgdRowCrossFields,
      isValuePresent,
  ...
  const shapData = useMemo(() => {
    ...
    if (!row) return null;
    return buildRetailLoanLgdShapData(row);
    ...
    <div className="simulation-panel">
      ...
      {isInitialLoading ? (
        <div className="loading-spinner">
          ...
          <p className="loading-text">Loading retail loans from backend...</p>
        </div>
      ) : apiError ? (
        ...
        <div className="loading-spinner">
          <div className="spinner"></div>
          <p className="loading-text">Running Retail Loan LGD model...</p>
        </div>
      ) : (
        <>
          <RetailLoanLgdDataTable
            rows={rows}
            draftRows={draftRows}
...
export default RetailLoanLgdSimulationPanel;
