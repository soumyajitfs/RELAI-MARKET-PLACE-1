...
const ChurnSimulationPanel = () => {
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFieldRanges(dirtyCells, draftCustomers, FIELD_RANGES, {
      getRowId: (row) => row.__rowId,
      formatError: ({ item, range, isTypeInvalid }) =>
        isTypeInvalid
          ? `"${range.label}" for User ${item.user_id} must be an integer.`
          : `"${range.label}" for User ${item.user_id} must be between ${range.min} and ${range.max}.`,
    });
    if (error) {
