...
import { PERSONAL_LOAN_EAD_INPUT_COLUMN_ORDER } from '../../data/personalLoanEadFieldRules';
import { resolvePredictedKFactor } from '../../utils/personalLoanEadApi';
...
const formatKFactor = (row) => {
  const k = resolvePredictedKFactor(row);
  if (k == null || !Number.isFinite(Number(k))) return '—';
  return Number(k).toFixed(4);
};

const PersonalLoanEadDataTable = ({
  rows,
  draftRows,
  setDraftRows,
  markCellDirty,
  ...
  const inputColumns = useMemo(() => {
    ...
    if (!sample) return [...PERSONAL_LOAN_EAD_INPUT_COLUMN_ORDER];
    return PERSONAL_LOAN_EAD_INPUT_COLUMN_ORDER.filter((key) => key in sample || fieldRules?.[key]);
  ...
  const renderCell = (row, key) => {
    const value = row[key];
    if (!editEnabled) {
      if (['topup_flag', 'restructured_flag', 'original_tenure_months'].includes(key) && value != null && value !== '') {
        ...
        >
          <thead>
            <tr>
              ...
              <th style={{ width: showInputCols ? '120px' : '15%' }}>Predicted K-Factor</th>
              <th style={{ width: showInputCols ? '140px' : '18%' }}>Predicted EAD (INR)</th>
              {showInputCols &&
                inputColumns.map((key) => (
            ...
            {displayRows.map((row) => {
              ...
              return (
                <tr key={`out-${row.__rowId}`} className={`${isSelected ? 'selected' : ''} ${isFrozen ? 'frozen' : ''}`}>
                  ...
                  <td>
                    <span style={{ color: '#1e3a5f', fontWeight: 600 }}>{formatKFactor(row)}</span>
                  </td>
                  <td>
                    <span style={{ color: '#1e3a5f', fontWeight: 600 }}>{formatInr(row.predicted_ead)}</span>
                  </td>
...
export default PersonalLoanEadDataTable;
