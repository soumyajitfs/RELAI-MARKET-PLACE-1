...
const SimulationPanel = () => {
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFieldRanges(dirtyCells, draftAccounts, FIELD_RANGES, {
      getRowId: (acc) => acc.facsNumber,
      formatError: ({ item, range }) =>
        `"${range.label}" for FACS ${item.facsNumber} must be between ${range.min} and ${range.max}. Please correct the value before applying.`,
    });
    if (error) {
