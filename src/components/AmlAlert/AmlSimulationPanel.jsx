...
import { validateDirtyFieldRanges } from '../../utils/dirtyCellTracking';
...
const AmlSimulationPanel = () => {
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFieldRanges(dirtyCells, draftAccounts, FIELD_RANGES, {
      getRowId: (acc) => acc.__rowId,
      formatError: ({ item, range, isTypeInvalid }) =>
        isTypeInvalid
          ? `"${range.label}" for Account ${item.accountId} must be an integer.`
          : `"${range.label}" for Account ${item.accountId} must be between ${range.min} and ${range.max}.`,
    });
    if (error) {
