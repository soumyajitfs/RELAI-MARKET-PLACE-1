...
import {
  fetchBehaviouralScorecardAccounts,
  predictBehaviouralScorecardAccounts,
} from '../../utils/behaviouralScorecardApi';
import { buildBehaviouralScorecardShapData } from '../../utils/behaviouralScorecardShapUtils';
import {
  resolveBehaviouralScorecardFieldRules,
  validateBehaviouralScorecardValue,
  validateBehaviouralScorecardRowCrossFields,
...
import BehaviouralScorecardDataTable from './BehaviouralScorecardDataTable';
...
const BehaviouralScorecardSimulationPanel = () => {
  ...
  const loadRows = useCallback(async () => {
    ...
    setApiError(null);
    try {
      const data = await fetchBehaviouralScorecardAccounts();
      setRows(data);
    } catch (err) {
      console.error('Failed to fetch behavioural accounts:', err);
  ...
  const fieldRulesWithEdit = useMemo(() => {
    const base = resolveBehaviouralScorecardFieldRules();
    const sample = rows[0];
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const results = await predictBehaviouralScorecardAccounts(rowsToRun);
      ...
      console.error('Behavioural Scorecard prediction failed:', err);
  ...
  const validateField = useCallback(
    (field, value) => validateBehaviouralScorecardValue(field, value, fieldRulesWithEdit),
    [fieldRulesWithEdit],
  );
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFieldRules({
      ...
      validateValue: validateBehaviouralScorecardValue,
      validateRowCross: validateBehaviouralScorecardRowCrossFields,
      isValuePresent,
      formatError: ({ row, field, rule, type, message }) => {
        if (type === 'empty') return `"${rule.label}" cannot be empty. (${row.cust_id})`;
        if (type === 'cross') return `${message} (${row.cust_id})`;
        return `${message} (${row.cust_id})`;
      },
    });
    if (error) {
  ...
  const shapData = useMemo(() => {
    ...
    if (!row) return null;
    return buildBehaviouralScorecardShapData(row);
    ...
    <div className="simulation-panel">
      ...
      <p className="simulation-caption">
        ...
        {hasOutput && rows.find((r) => r.__rowId === selectedRowId) && (
          <strong> ({rows.find((r) => r.__rowId === selectedRowId).cust_id} selected)</strong>
        )}
      </p>
      ...
      {isInitialLoading ? (
        <div className="loading-spinner">
          ...
          <p className="loading-text">Loading behavioural accounts from backend...</p>
        </div>
      ) : apiError ? (
        ...
        <div className="loading-spinner">
          <div className="spinner"></div>
          <p className="loading-text">Running Behavioural Scorecard model...</p>
        </div>
      ) : (
        <>
          <BehaviouralScorecardDataTable
            rows={rows}
            draftRows={draftRows}
...
export default BehaviouralScorecardSimulationPanel;
