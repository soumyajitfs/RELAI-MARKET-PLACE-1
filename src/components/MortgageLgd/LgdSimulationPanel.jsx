...
import { fetchLgdAccounts, predictLgdAccounts } from '../../utils/lgdApi';
import { buildLgdShapData } from '../../utils/lgdShapUtils';
import { resolveLgdFieldRules, validateLgdValue, validateLgdRowCrossFields } from '../../data/lgdFieldRules';
import { useDirtyCells } from '../../hooks/useDirtyCells';
import { validateDirtyFieldRules } from '../../utils/dirtyCellTracking';
import LgdDataTable from './LgdDataTable';
...
const LgdSimulationPanel = () => {
  ...
  const loadRows = useCallback(async () => {
    ...
    setApiError(null);
    try {
      const data = await fetchLgdAccounts();
      setRows(data);
    } catch (err) {
  ...
  const fieldRulesWithEdit = useMemo(() => {
    const base = resolveLgdFieldRules();
    const sample = rows[0];
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const results = await predictLgdAccounts(rowsToRun);
      ...
      console.error('LGD prediction failed:', err);
  ...
  const validateField = useCallback(
    (field, value) => validateLgdValue(field, value, fieldRulesWithEdit),
    [fieldRulesWithEdit]
  );
  ...
  const handleApplyChanges = () => {
    ...
    const error = validateDirtyFieldRules({
      ...
      validateValue: validateLgdValue,
      validateRowCross: validateLgdRowCrossFields,
      isValuePresent,
  ...
  const shapData = useMemo(() => {
    ...
    if (!row) return null;
    return buildLgdShapData(row);
  }, [hasOutput, selectedRowId, rows]);

  return (
    <div className="simulation-panel lgd-simulation">
      ...
      {isInitialLoading ? (
        <div className="loading-spinner">
          ...
          <p className="loading-text">Loading LGD accounts from backend...</p>
        </div>
      ) : apiError ? (
        ...
        <div className="loading-spinner">
          <div className="spinner"></div>
          <p className="loading-text">Running LGD model...</p>
        </div>
      ) : (
        <>
          <LgdDataTable
...
export default LgdSimulationPanel;
